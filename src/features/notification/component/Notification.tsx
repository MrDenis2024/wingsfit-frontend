...
const Notification:React.FC<Props> = ({message, courseToday, endedSubscription, lesson})=> {
    const user = useAppSelector(selectUser);
    let content: React.ReactNode;

    if (message) {
        content = (
            <Link href={`/clients/chats/${user?._id}`} sx={{
                p: 1,
    ...
    if (endedSubscription) {
        content = (
            <Link href={`/courses/${endedSubscription.courseId}`} sx={{
                p: 1,
                border: '1px solid gray',
                borderRadius: '5px',
                width: '100%',
    ...
    if (courseToday) {
        content = (
            <Link href={`/courses/${courseToday.course._id}`} sx={{
                p: 1,
                border: '1px solid gray',
                borderRadius: '5px',
                width: '100%',
                ...
                <ListItemText
                    ...
                    secondary={
                        `У вашей группы сегодня занятие в - ${courseToday.group.startTime}`
                    }
                />
            </Link>
        );
    ...
    if (lesson) {
        content = (
            <Link href={`/courses/${lesson.group.course._id}`} sx={{
                p: 1,
                border: '1px solid gray',
                borderRadius: '5px',
                width: '100%',
                ...
                <ListItemText
                    ...
                    secondary={
                        `У вашей группы началось занятие`
                    }
                />
            </Link>
        );
