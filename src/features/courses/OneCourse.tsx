...
const OneCourse = () => {
  ...
  const handleClickEditCourse = () => {
    ...
    const userIsCandidate = course?.waitList.find(item=>item.user._id===user?._id);
    let userIsClient = false;
    groups.forEach((group) => {
        const userSubscriber = group.clients.find(client=>client.client._id===user?._id);
        if (userSubscriber) {
            userIsClient = true;
        }
    });
        ...
        <Grid>
          ...
          >
            {!loadingGroups ? (
              groups.length > 0 ? (
                groups.map((group) => (
                    <CoursesGroupCards key={group._id} group={group} waitListItem={userIsCandidate} courseId={course._id} userIsClient={userIsClient}/>
                ))
              ) : (
